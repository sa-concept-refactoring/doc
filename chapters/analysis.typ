...
/ Apply a WorkspaceEdit: #[
  ...
  When the client then applies the provided changes and reports back to the server whether the job has been successfully applied or not.
...
For a compiler to transform source code into compiled code, three steps are needed to do so.
...
Clang has a builtin AST-dump mode, which can be enabled with the `-ast-dump` flat. @clang_ast
